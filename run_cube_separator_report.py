...
def load_quality_v2(path: Path):
    ...
    Supports both single-model bundles (model_state_dict)
    and K-Fold ensemble bundles (fold_state_dicts).
    """
    bundle      = torch.load(path, map_location="cpu", weights_only=False)
    ...
    if "fold_state_dicts" in bundle:
        # K-Fold ensemble — load all fold models
        nets = []
        for sd in bundle["fold_state_dicts"]:
            net = _make_efficientnet(num_classes=num_classes)
            net.load_state_dict(sd)
            net.eval()
            nets.append(net.to(device))
        k         = bundle.get("k_folds", len(nets))
        mean_acc  = bundle.get("mean_val_acc", "?")
        fold_accs = [round(a, 3) for a in bundle.get("fold_val_accs", [])]
        print(f"Quality model loaded | {k}-fold ensemble | {arch} | input={input_size}px"
              f" | mean_val_acc={mean_acc} | folds={fold_accs}")
        return nets, idx_to_class, device, "v2_ensemble", _build_transform(input_size), _build_tta_transforms(input_size)
    else:
        # Single model (legacy)
        net = _make_efficientnet(num_classes=num_classes)
        net.load_state_dict(bundle["model_state_dict"])
        ...
        print(f"Quality model loaded | v{version} | {arch} | input={input_size}px"
              f" | val_acc={bundle.get('val_acc','?')} | classes={bundle.get('classes')}")
...
def quality_score_v2(net_or_nets, idx_to_class, device, transform, tta_transforms, img: Image.Image, use_tta: bool = False):
    """V2/V3: EfficientNet 3-class — returns (result, confidence, margin).

    Accepts either a single net or a list of nets (K-Fold ensemble).
    Ensemble: averages softmax probabilities across all fold models.
    TTA: averages predictions over 5 crops per model.
    margin = top-1 probability minus top-2 probability.
    """
    nets = net_or_nets if isinstance(net_or_nets, list) else [net_or_nets]
